...
// Use process.cwd() for directory resolution in CommonJS context
...
export class WorkerThreadPool implements WorkerPool<Worker> {
    ...
    private createWorker(): Worker {
        const workerPath = path.join(currentDir, 'template-worker.js');
...
if (!isMainThread && parentPort) {
    parentPort.on('message', async (task: WorkerTask<any, ProcessingResult>) => {
        try {
 ...
 */
async function renderTemplate(template: Template, context: GeneratorContext): Promise<string> {
 ...
 */
function generateOutputPath(template: Template, context: GeneratorContext): string {
