...
async function bumpVersion() {
  try {
    ...
    const vscodePackageJsonPath = path.join(rootDir, '..', 'vscode-extension', 'package.json');
    if (await fs.pathExists(vscodePackageJsonPath)) {
      const vscodePackageJson = await fs.readJson(vscodePackageJsonPath);
      vscodePackageJson.version = newVersion;
      await fs.writeJson(vscodePackageJsonPath, vscodePackageJson, { spaces: 2 });
      console.log(chalk.blue(`📝 Updated VS Code extension version to ${newVersion}`));
    }
