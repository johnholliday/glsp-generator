...
export class TemplateResolver {
    ...
    resolveTemplates(context: TemplateContext): GenerationItem[] {
        ...
        // Define Theia-specific templates to exclude in vsixOnly mode
        const theiaSpecificTemplates = [
            'glsp-diagram-module',
            'glsp-model-factory',
            'glsp-model-state',
            'glsp-server',
            'glsp-views',
            'server-diagram-configuration',
            'server-model',
            'create-node-handler',
            'yarn.lock'
        ];
        ...
        for (const [templateName, compiledTemplate] of this.templateSet.templates) {
            // Skip Theia-specific templates if vsixOnly is true
            if (context.vsixOnly && theiaSpecificTemplates.includes(templateName)) {
                continue;
            }
    ...
    private resolveFileName(templateName: string, context: TemplateContext): string {
        ...
        if (templateName === 'readme' || templateName === 'README.md' || templateName === 'README') {
            return 'README.md';
        }
