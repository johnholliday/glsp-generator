...
export class GenerateCommand extends BaseCommand<GenerateArgs> {
  ...
  builder(yargs: Argv): Argv<GenerateArgs> {
    return yargs
      ...
      .option('vsix', {
        describe: 'Package as VSIX after generation',
        type: 'boolean',
        default: false
      })
      .option('vsix-only', {
        describe: 'Generate only VS Code extension (no Theia components)',
        type: 'boolean',
        default: false
      })
      .option('no-subdir', {
        describe: 'Generate directly to output directory without creating a subdirectory',
        type: 'boolean',
        default: false
      })
      .example('$0 gen state-machine.langium', 'Generate project files to ./output')
      .example('$0 gen grammar.langium ./my-extension', 'Generate to custom directory')
      .example('$0 gen grammar.langium --vsix', 'Generate and package as VSIX')
  ...
  async handler(args: GenerateArgs): Promise<void> {
    try {
      ...
      if (args.dev) {
      ...
      } else if (args.vsix || args.debug) {
        // Generate VSIX package when explicitly requested or in debug mode
        const tempDir = await this.tempDirManager.createTempDirectory('glsp-gen-');
        try {
          // Generate to temp directory
          const extensionDir = await this.generateExtension(grammarPath, tempDir.path, args);
          
          // Package as VSIX using the actual extension directory
          const packageResult = await this.vsixPackager.packageExtension({
            projectPath: extensionDir,
            outputPath,
            logger: this.logger
          });
        ...
        // Default: generate project files to output directory
        await this.checkOutputDirectory(outputPath, args.force);
        await this.generateExtension(grammarPath, outputPath, args);
  ...
  private async generateExtension(
    ...
    const { extensionDir } = await this.generator.generateExtension(grammarPath, outputPath, {
      ...
      vsixOnly: args['vsix-only'],
      noSubdir: args['no-subdir']
    });
