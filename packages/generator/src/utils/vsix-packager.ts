...
export class VsixPackager {
  ...
  async packageExtension(options: VsixPackageOptions): Promise<VsixPackageResult> {
    ...
    try {
      ...
      const vsceCommand = process.env.API_PORT ? 'vsce' : 'npx';
      const vsceArgs = process.env.API_PORT 
        ? ['package', '--yarn'] 
        : ['vsce', 'package', '--yarn'];
      
      await this.runCommand(vsceCommand, vsceArgs, projectPath);
