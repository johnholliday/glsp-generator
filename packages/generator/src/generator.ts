...
export class GLSPGenerator {
  ...
  async generateExtension(
  ...
  ): Promise<{ success: boolean; extensionDir: string }> {
    ...
    try {
      ...
      let defaultLogoPath: string | undefined = path.join(path.dirname(grammarFile), 'icon.png');
      if (!await fs.pathExists(defaultLogoPath)) {
        // Try logo.png in the grammar directory
        defaultLogoPath = path.join(path.dirname(grammarFile), 'logo.png');
        if (!await fs.pathExists(defaultLogoPath)) {
          // Use the logo from the vscode extension
          defaultLogoPath = path.join(getTemplatesDir(), '..', '..', '..', 'vscode-extension', 'logo.png');
          if (!await fs.pathExists(defaultLogoPath)) {
            // If still not found, set to undefined
            defaultLogoPath = undefined;
          }
        }
      }
      ...
      const packageJsonData = VSIXMetadataExtractor.toPackageJson(
        vsixMetadata,
        parsedGrammar.projectName,
        this.config.extension.fileExtension || parsedGrammar.projectName
      );
      
      // If icon is present, use only the filename (not the full path)
      if (packageJsonData.icon) {
        packageJsonData.icon = path.basename(packageJsonData.icon);
      }
      ...
      const extensionDir = options?.noSubdir 
        ? outputDir 
        : path.join(outputDir, extensionName);

      // Phase 2: Setup
      progress.startPhase('Setup');
      console.log(chalk.blue('📁 Creating project structure...'));
      await this.createProjectStructure(extensionDir, options?.vsixOnly);
      ...
      await this.generateFiles(context, extensionDir, shouldOptimize, options?.vsixOnly);
  ...
  async generate(
    ...
    return this.generateExtension(grammarFile, outputDir, options);
  }

  private async createProjectStructure(extensionDir: string, vsixOnly = false): Promise<void> {
    const vsCodeDirectories = [
      'src',
      'src/extension',
      'src/webview',
      'syntaxes',
    ...
    const directories = vsixOnly ? vsCodeDirectories : fullDirectories;
  ...
  private async generateFiles(context: GenerationContext, extensionDir: string, useOptimizations = false, vsixOnly = false): Promise<void> {
